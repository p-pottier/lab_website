 ...
 * Earth atlas. The first two entries are the mismatches present in the current
 * data; the rest guard against spellings that appear once a new co-author is
 * added. Every right-hand value was checked against countries-110m.json.
 */
...
export default function CollaboratorMap() {
  ...
  return (
    <div>
      ...
      <div
        ref={wrapRef}
        className="relative mt-5 overflow-hidden rounded-2xl border border-edge bg-[#111]"
      >
        <svg
        ...
        >
          ...
          <g>
            {features.map((f, i) => {
              ...
              const fill =
                n === 0 ? "#1E1E1E" : rampColour(Math.log1p(n) / Math.log1p(maxCount));
          ...
          <g transform={`translate(${WIDTH - 210}, ${HEIGHT - 42})`}>
            <text x={0} y={-6} fontSize="11" fill="#9CA3AF" fontWeight={600}>
              Co-authors per country
            </text>
            <rect width={170} height={9} rx={4} fill="url(#legendRamp)" />
            <text x={0} y={22} fontSize="10" fill="#6B7280">
              1
            </text>
            <text x={158} y={22} fontSize="10" fill="#6B7280">
              {maxCount}
            </text>
          </g>
        </svg>
        ...
        {hover && (
          <div
            className="pointer-events-none absolute z-10 -translate-x-1/2 -translate-y-[130%] rounded-lg border border-gold/40 bg-ink/95 px-3 py-2 text-xs shadow-xl"
      ...
      {selected && (
        <div className="card mt-5 p-6">
          <div className="flex items-start justify-between gap-4">
            <div>
              <h3 className="font-display text-xl font-semibold text-white">{selected}</h3>
          ...
          <ul className="mt-5 grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
            {inCountry.map((c) => (
              <li key={c.name} className="rounded-lg border border-white/5 bg-ink/50 p-3">
                {c.orcid ? (
                  <a
                    href={c.orcid}
                    target="_blank"
                    rel="noreferrer"
                    className="text-sm font-medium text-white transition hover:text-gold"
                  >
                    {c.name}
                  </a>
                ) : (
